...
class GeoLocation(Base):
    ...
    home_location: Mapped[Geometry] = mapped_column(Geometry(geometry_type='POINT', srid=4326), nullable=True)
    radius: Mapped[int] = mapped_column(nullable=True)
    polygon: Mapped[Geometry] = mapped_column(Geometry(geometry_type='POLYGON', srid=4326), nullable=True)
